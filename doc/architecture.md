...
```
┌───────────┐   ┌────────┐   ┌───────┐   ┌───────────┐   ┌────────┐
│ Gmail API │──▶│ Parser │──▶│ Dedup │──▶│ Summarize │──▶│ Output │
│ (fetch)   │   │ (HTML) │   │(group)│   │ (Ollama)  │   │(Slack) │
└───────────┘   └────────┘   └───────┘   └───────────┘   └────────┘
```
...
4. **Summarize**: `summarizer.py` generates 3-bullet summaries (problem, technical approach, finding) of each paper's abstract using a local LLM (Ollama/qwen3:8b)
5. **Output**: `output.py` prints to console, `slack_output.py` sends formatted digest to Slack (summary + threaded details)
